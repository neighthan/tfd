...
def instantiate_factored_mapping(pairs):
    part_mappings = [
        [list(zip(preimg, perm_img)) for perm_img in tools.permutations(img)]
...
class SafeInvariant(Invariant):
    ...
    def check_balance(self, balance_checker, enqueue_func):
        ...
        for action in actions_to_check:
            ...
            if self.operator_too_heavy(heavy_action):
                return False
            unbalanced, new_candidates = self.operator_unbalanced(
                action, temp_unbalanced_actions
            )
            if unbalanced:
    ...
    def operator_too_heavy(self, h_action):
        ...
        for time in range(2):
            ...
            add_effects = [
                eff
                for eff in h_action.effects[time]
    ...
    def operator_unbalanced(self, action, temp_unbalanced_actions):
        ...
        for time in range(2):
            ...
            add_effects[time] = [
                eff for eff in relevant_effs[time] if not eff.peffect.negated
            ]
            del_effects[time] = [
                eff for eff in relevant_effs[time] if eff.peffect.negated
            ]
        for time in range(2):
            poss_temporary_cand = (time == 1) and not len(add_effects[0])
            for eff in add_effects[time]:
                unbal, new_candidates = self.add_effect_unbalanced(
                    action, eff, del_effects[time], inv_vars, time
                )
                if unbal:
                    ...
                    if poss_temporary_cand:
                        unbal, new_cands = self.add_effect_temporarily_unbalanced(
                            action, eff, del_effects[0], inv_vars
                        )
                        if unbal:
    ...
    def add_effect_unbalanced(self, action, add_effect, del_effects, inv_vars, time):
        ...
        for lit in itertools.chain(
            get_literals(action.condition[cond_time]),
        ...
        for del_effect in del_effects:
            if time == 1 and (del_effect.condition[0] or del_effect.condition[1]):
                continue
            minimal_renamings = self.unbalanced_renamings(
                del_effect, add_effect, inv_vars, lhs_by_pred, time, minimal_renamings
            )
    ...
    def add_effect_temporarily_unbalanced(
        self, action, add_effect, start_del_effects, inv_vars
    ):
        ...
        lhs_by_pred = defaultdict(list)
        for lit in itertools.chain(
            get_literals(action.condition[0]),
        ...
        for del_effect in start_del_effects:
            minimal_renamings = self.temp_unbalanced_renamings(
    ...
    def temp_unbalanced_renamings(
        ...
        for renaming in unbalanced_renamings:
            ...
            if self.lhs_satisfiable(renaming, lhs_by_pred):
                implies_system = self.imply_del_effect(del_effect, lhs_by_pred, 0)
    ...
    def unbalanced_renamings(
        self, del_effect, add_effect, inv_vars, lhs_by_pred, time, unbalanced_renamings
    ):
        """returns the renamings from unbalanced renamings for which
        ...
        for renaming in unbalanced_renamings:
            new_sys = system.combine(renaming)
            if self.lhs_satisfiable(renaming, lhs_by_pred):
                implies_system = self.imply_del_effect(del_effect, lhs_by_pred, time)
                if not implies_system:
                    still_unbalanced.append(renaming)
                    continue
                new_sys = new_sys.combine(implies_system)
    ...
    def imply_del_effect(self, del_effect, lhs_by_pred, time):
        ...
        for literal in itertools.chain(
            get_literals(del_eff_condition), [del_effect.peffect.negate()]
        ):
            poss_assignments = []
    ...
    def conditions_require_weight_1(self, action, add_effect):
        ...
        relevant_conditions = [
            ...
            if not atom.negated and self.predicate_to_part.get(atom.predicate)
        ]
...
class UnsafeInvariant(Invariant):
    ...
    def check_action_balance(self, balance_checker, action, enqueue_func):
        ...
        if isinstance(action, pddl.Action):
            del_effects = [
                ...
                if isinstance(eff.peffect, pddl.NegatedAtom)
            ]
            add_effects = [
                eff for eff in action.effects if isinstance(eff.peffect, pddl.Atom)
            ]
            matched_add_effects = []
            for eff in add_effects:
                ...
                if part:
                    for previous_part, previous_peffect in matched_add_effects:
                        if (
                            previous_part.matches(part, previous_peffect, eff.peffect)
                            and previous_peffect != eff.peffect
                        ):  # "Airport" has duplicate effects
                            return False  # operator too heavy
                    if not self.find_matching_del_effect(
                        part, eff, del_effects, enqueue_func
                    ):
                        return False
            ...
            start_add_effects = [
                eff for eff in action.effects[0] if isinstance(eff.peffect, pddl.Atom)
            ]
            end_add_effects = [
                ...
                if part:
                    for previous_part, previous_peffect in matched_start_add_effects:
                        if (
                            ...
                            return False  # operator too heavy
                    if not self.find_matching_del_effect(
                        part, eff, start_del_effects, enqueue_func
                    ):
                        return False
            ...
            for eff in end_add_effects:
                ...
                if part:
                    ...
                    if check_all_del_effects:
                        found_start_del = self.find_matching_del_effect(
                            part, eff, start_del_effects, enqueue_func, False
                        )
                    found_end_del = self.find_matching_del_effect(
                        part, eff, end_del_effects, enqueue_func, False
                    )
                    if not (found_start_del or found_end_del):
                        if not found_end_del:
                            self.generate_new_candidates(
                                part, eff, end_del_effects, enqueue_func
                            )
    ...
    def find_matching_del_effect(
        self, part, add_effect, del_effects, enqueue_func, generate_new=True
    ):
