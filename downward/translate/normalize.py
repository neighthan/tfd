...
class PreconditionProxy(ConditionProxy):
    ...
    def build_rules(self, rules, fluent_preds):
        ...
        if isinstance(self.condition, list):
            rule_body = list(
                condition_to_rule_body(action.parameters, self.condition[0])
            )
            rule_body += list(
                condition_to_rule_body(
                    action.parameters, self.condition[1], fluent_preds
                )
            )
            rule_body += list(
                condition_to_rule_body(
                    action.parameters, self.condition[2], fluent_preds
                )
            )
        else:
...
class AxiomConditionProxy(ConditionProxy):
    ...
    def build_rules(self, rules, fluent_preds):
        ...
        eff_rule_head = pddl.Atom(
            axiom.name, [pddl.Variable(par.name) for par in axiom.parameters]
        )
    ...
    def recurse(condition):
        ...
        if isinstance(condition, pddl.UniversalCondition):
            ...
            return pddl.NegatedAtom(
                axiom.name, [pddl.conditions.parse_term(par) for par in parameters]
            )
        else:
    ...
    for act in task.durative_actions:
        ...
        for time in range(2):
            for index, (op, exp) in enumerate(act.duration[time]):
                typed_vars, function_terms, new_term = exp.compile_objectfunctions_aux(
                    used_variables, recurse_object_terms=False
                )
...
def remove_duration_variable(task):
    def recurse(condition, act, time, duration, pnes):
        if isinstance(condition, pddl.FunctionComparison):
            ...
            new_parts = [
                recurse(part, act, time, duration, pnes) for part in condition.parts
            ]
            return condition.change_parts(new_parts)
        ...
        for time in range(2):
            for eff in act.effects[time]:
                ...
                for eff_time, cond in enumerate(eff.condition):
                    condition.append(
                        recurse(cond, act, eff_time, duration, duration_functions)
                    )
