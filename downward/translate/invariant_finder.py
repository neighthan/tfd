    ...
    def add_inequality_preconds(self, action, reachable_action_params):
        ...
        if new_cond_parts:
            ...
            for time in (0, 2):  # add inequalities to start and end condition
                ...
                if isinstance(action.condition[time], pddl.Literal) or isinstance(
                    action.condition[time], pddl.FunctionComparison
                ):
                    cond_parts = [action.condition[time]]
                cond_parts.extend(new_cond_parts)
                cond = pddl.Conjunction(cond_parts)
                new_cond[time] = cond
            return pddl.DurativeAction(
                action.name,
                action.parameters,
                action.duration,
                new_cond,
