...
def is_function_comparison(alist):
    ...
    if isinstance(symbol, list):
        ...
        symbol = symbol[0]
    if (
        tasks.Task.FUNCTION_SYMBOLS.get(symbol, "object") == "number"
...
class Literal(Condition):
    ...
    def uniquify_variables(self, type_map, renamings={}):
        if not self.args:
            ...
            return self.__class__(
                self.predicate,
                [arg.uniquify_variables(type_map, renamings) for arg in self.args],
            )
            ...
            if new_var_name not in used_variables:
                ...
                typed_vars.append(
                    pddl_types.TypedObject(
                        new_var_name, tasks.Task.FUNCTION_SYMBOLS[self.name]
                    )
                )
