...
class Task(object):
    ...
    def parse(domain_pddl, task_pddl):
        ...
        init += [
            conditions.Atom(
                "=", (conditions.parse_term(obj.name), conditions.parse_term(obj.name))
            )
            for obj in objects
        ]
        return Task(
...
class DerivedFunctionAdministrator(object):
    ...
    def get_derived_function(self, exp):
        ...
        if isinstance(exp, f_expression.PrimitiveNumericExpression):
            ...
            assert (
                isinstance(exp, f_expression.ArithmeticExpression)
                and len(exp.parts) == 2
            )
            ...
            if key not in self.functions:
                ...
                if key not in self.functions:
                    ...
                    pne1 = f_expression.PrimitiveNumericExpression(
                        pne1.symbol, default_args[: len(pne1.args)]
                    )
                    if pne2.args:
                        pne2 = f_expression.PrimitiveNumericExpression(
                            pne2.symbol, default_args[-len(pne2.args) :]
                        )

                    self.functions[key] = axioms.NumericAxiom(
                        symbol, tuple(default_args), exp.op, [pne1, pne2]
                    )
...
def parse_domain(domain_pddl):
    ...
    if opt_types[0] == ":types":
        the_types.extend(
            pddl_types.parse_typed_list(opt_types[1:], constructor=pddl_types.Type)
        )
    ...
    the_types = [
        ...
        if type.supertype_names != [] or type.name == "object"
    ]
...
def parse_task(task_pddl):
    ...
    for fact in init[1:]:
        if fact[0] == "=":
            ...
            initial.append(
                conditions.Atom(
                    fact[0], [conditions.parse_term(term) for term in fact[1:]]
                )
            )
    yield initial
