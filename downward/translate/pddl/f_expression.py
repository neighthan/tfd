...
class ArithmeticExpression(FunctionalExpression):
    ...
    def remove_duration_variable(self, action, time, duration, pnes):
        return self.__class__(
            [
                part.remove_duration_variable(action, time, duration, pnes)
                for part in self.parts
            ]
        )
...
class PrimitiveNumericExpression(FunctionalExpression):
    ...
    def free_variables(self):
        return set(
            arg.name for arg in self.args if isinstance(arg, conditions.Variable)
        )
    ...
    def instantiate(
        ...
        args = [
            var_mapping.get(conditions.Variable(arg.name), arg) for arg in self.args
        ]
