...
def parse_effect(alist, durative=False):
    ...
    if tag == "and":
        ...
        return UniversalEffect(
            pddl_types.parse_typed_list(alist[1]), parse_effect(alist[2], durative)
        )
    elif tag == "when":
        assert len(alist) == 3
        if durative:
        ...
        new_alist = [
            "and",
            ["at", "start", ["assign", alist[1], "undefined"]],
...
def parse_cond_effect(alist, durative=False):
    ...
    if tag == "and":
        return ConjunctiveEffect(
            [parse_cond_effect(eff, durative) for eff in alist[1:]]
        )
            ...
            return ConjunctiveEffect(
                [
                    ObjectFunctionAssignment(
                        conditions.parse_term(alist[1]), conditions.parse_term(alist[2])
                    )
                ]
            )
    else:
...
class Effect(object):
    ...
    def __hash__(self):
        return hash((self.__class__, tuple(self.parameters), tuple(self.condition), self.peffect))
    ...
    def dump(self):
        ...
        if self.parameters:
            ...
            indent += "  "
        if (
            isinstance(self.condition, list)
            and self.condition
            != [conditions.Truth(), conditions.Truth(), conditions.Truth()]
        ) or (
            not isinstance(self.condition, list)
            and self.condition != conditions.Truth()
        ):
            print("%sif" % indent)
    ...
    def uniquify_variables(self, type_map):
        ...
        if isinstance(self.condition, list):
            self.condition = [
                cond.uniquify_variables(type_map, renamings) for cond in self.condition
            ]
        else:
        ...
        if self.parameters:
            ...
            for object_tuple in cartesian_product(*object_lists):
                for (par, obj) in zip(self.parameters, object_tuple):
                    var_mapping[conditions.Variable(par.name)] = conditions.ObjectTerm(
                        obj
                    )
                self._instantiate(
                    var_mapping,
...
class ConditionalEffect(TmpEffect):
            ...
            if len(condition) == 1:
                ...
                for index, cond in enumerate(condition):
                    new_condition.append(
                        conditions.Conjunction([cond, effect.condition[index]])
                    )
    ...
    def normalize(self):
        ...
        for effect in effects:
            ...
            if isinstance(effect, ConjunctiveEffect) or isinstance(
                effect, ConditionalEffect
            ):
                new_effects.append(
                    ConditionalEffect(self.condition, effect, effect.time)
                )
...
class UniversalEffect(TmpEffect):
    ...
    def normalize(self):
        ...
        if effect.__class__.__name__ == "TmpEffect":
            return TmpEffect(
                [
                    UniversalEffect(self.parameters, eff, eff.time)
    ...
    def normalize(self):
        ...
        for eff in self.effects:
            if isinstance(eff, ObjectFunctionAssignment):
                ...
                assert isinstance(eff, f_expression.FunctionAssignment) or isinstance(
                    eff, conditions.Literal
                )
...
class ObjectFunctionAssignment(object):
    ...
    def rename_variables(self, renamings):
        return self.__class__(
            self.head.rename_variables(renamings),
    ...
    def normalize(self, time, results):
        ...
        add_params = set(
            [typed for typed in typed1 if not typed.name == term1.name] + typed2
        )
        ...
        if term2.name != "undefined":
            del_undef_params = set(
                [typed for typed in del_params if not typed.name == del_param.name]
            )
            atom_parts = list(conjunction_parts1[0].args)
            ...
            if len(del_undef_conjunction_parts) > 0:
                del_undef_effect = ConditionalEffect(
                    del_undef_condition, del_undef_effect, time
                )
            if len(del_undef_params) > 0:
                del_undef_effect = UniversalEffect(
                    del_undef_params, del_undef_effect, time
                )
