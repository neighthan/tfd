...
class Action(object):
    ...
    def instantiate(
        ...
        arg_list = [
            var_mapping[conditions.Variable(par.name)].name for par in self.parameters
        ]
...
class DurativeAction(object):
    ...
    def parse(alist):
        ...
        return DurativeAction(
            name, parameters, (duration_start, duration_end), condition, effect
        )
        ...
        if self.orig_parameter_length != len(self.parameters):
            print(
                ...
                % (
                    self.name,
                    ", ".join(
                        map(str, self.parameters[0 : self.orig_parameter_length])
                    ),
    ...
    def instantiate(
        ...
        if effects:
            return PropositionalDurativeAction(
                name, inst_duration, inst_conditions, effects
            )
        else:
            return None
